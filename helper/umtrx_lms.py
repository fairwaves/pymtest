...
RESV_REGS = (0x0C, 0x0D, 0x37, 0x38, 0x39, 0x3A, 0x3B,
             0x3C, 0x3D, 0x69, 0x6A, 0x6B, 0x6C, 0x6D)


def dump(lms_dev):
    return [(x, lms_dev.reg_read(x),)
            for x in range(0, 128) if x not in RESV_REGS]


def select_freq(freq):
    """ Test if given freq within the range and return corresponding value """
    l = list(filter(lambda t: True if t[0] < freq <= t[1] else False,
                    FREQ_LIST))
...
def lms_txrx_pll_tune(lms_dev, base_reg, ref_clock, out_freq):
    ...
    if verbosity > 0:
        print("FREQSEL=%d VCO_X=%d NINT=%d NFRAC=%d" %
              (freqsel, vco_x, nint, nfrac))

    # Write NINT, NFRAC
    lms_dev.reg_write(base_reg + 0x0, (nint >> 1) & 0xff)  # NINT[8:1]
    # NINT[0] NFRAC[22:16]
    lms_dev.reg_write(base_reg + 0x1,
                      ((nfrac >> 16) & 0x7f) | ((nint & 0x1) << 7))
    lms_dev.reg_write(base_reg + 0x2, (nfrac >> 8) & 0xff)  # NFRAC[15:8]
    lms_dev.reg_write(base_reg + 0x3, (nfrac) & 0xff)  # NFRAC[7:0]
    # Write FREQSEL
    lms_dev.reg_write_bits(base_reg + 0x5, (0x3f << 2),
                           (freqsel << 2))  # FREQSEL[5:0]
...
#    lms_dev.reg_write(base_reg+0x9, 0x94) # VOVCOREG[0] VCOCAP[5:0]
    ...
    if verbosity > 0:
        print("Actual frequency: %f" % (actual_freq))
...
def lms_general_dc_calibration_loop(lms_dev, dc_addr, calibration_reg_base):
    ...
    if verbosity > 0:
        print("DC Offset Calibration for addr %d:" % (dc_addr,))
...
def lms_general_dc_calibration(lms_dev, dc_addr, calibration_reg_base):
    ...
    # It's purpose is to circumvent the fact that in some edge cases
    # calibration may be successful even is DC_LOCK shows 0 or 7.

    # Set DC_REGVAL to 31
    lms_dev.reg_write(calibration_reg_base + 0x00, 31)
    # Run the calibration first time
    DC_REGVAL = lms_general_dc_calibration_loop(
        lms_dev, dc_addr, calibration_reg_base)
    ...
    if 31 == DC_REGVAL:
        ...
        if 0 == DC_REGVAL:
            ...
            # From LimeMicro FAQ: "[This] condition should not
            # happen as this is being checked in our production test."
            print("Error: DC Offset Calibration does not converge!")
            return None

    if verbosity > 0:
        print("Successful DC Offset Calibration for register bank 0x%X, DC " +
              "addr %d. Result: 0x%X" % (calibration_reg_base,
                                         dc_addr, DC_REGVAL))
    return DC_REGVAL
...
def lms_txrx_lpf_dc_calibration(lms_dev, is_tx):
    ...
    result = lms_general_dc_calibration(
        lms_dev, 0, control_reg_base) is not None
    # Perform DC Calibration Procedure in LPFSPI with ADDR := 1 (For channel Q)
    result = lms_general_dc_calibration(
        lms_dev, 1, control_reg_base) is not None and result
...
def lms_rxvga2_dc_calibration(lms_dev):
    ...
    # Perform DC Calibration Procedure in RxVGA2SPI with ADDR := 0 (For DC
    # Reference channel)
    result = lms_general_dc_calibration(
        lms_dev, 0, control_reg_base) is not None
    # Perform DC Calibration Procedure in RxVGA2SPI with ADDR := 1 (For
    # VGA2A_I channel)
    result = lms_general_dc_calibration(
        lms_dev, 1, control_reg_base) is not None and result
    # Perform DC Calibration Procedure in RxVGA2SPI with ADDR := 2 (For
    # VGA2A_Q channel)
    result = lms_general_dc_calibration(
        lms_dev, 2, control_reg_base) is not None and result
    # Perform DC Calibration Procedure in RxVGA2SPI with ADDR := 3 (For
    # VGA2B_I channel)
    result = lms_general_dc_calibration(
        lms_dev, 3, control_reg_base) is not None and result
    # Perform DC Calibration Procedure in RxVGA2SPI with ADDR := 4 (For
    # VGA2B_Q channel)
    result = lms_general_dc_calibration(
        lms_dev, 4, control_reg_base) is not None and result
...
def lms_lpf_bandwidth_tuning(lms_dev, ref_clock, lpf_bandwidth_code):
    ...
    # Save registers 0x05 and 0x09, because we will modify them during
    # lms_tx_enable()
    ...
    if verbosity >= 3:
        print("code = %x %x %x" %
              (lpf_bandwidth_code, t, lms_dev.reg_read(0x07)))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='UmTRX LMS debugging tool.',
                                     epilog="UmTRX is detected via " +
                                            "broadcast unless explicit " +
                                            "address is specified " +
                                            "via --umtrx-addr option. " +
                                            "'None' returned while " +
                                            "reading\writing indicates " +
                                            "error in the process.")
    parser.add_argument('--version', action='version', version='%(prog)s 3.2')
    parser.add_argument('--lms',
                        type=int,
                        choices=list(range(1, 3)),
                        help='LMS number, if no other options are given it ' +
                             'will dump all registers for corresponding LMS')
    parser.add_argument('--reg',
                        type=lambda s: int(s, 16),
                        choices=range(0, 0x80),
                        metavar='0..0x79',
                        help='LMS register number, hex')
    parser.add_argument('--verify', action='store_true',
                        help='read back written register value to verify ' +
                             'correctness')
    parser.add_argument('--pll-ref-clock', type=float, default=26e6,
                        help='PLL reference clock, 26MHz by default')
    parser.add_argument('--lpf-bandwidth-code',
                        type=lambda s: int(s, 16),
                        choices=range(0, 0x10),
                        metavar='0..0x0f',
                        help='LPF bandwidth code (default: 0x0f), used only ' +
                             'with --lms-lpf-bandwidth-tuning and ' +
                             '--lms-auto-calibration')
    basic_opt = parser.add_mutually_exclusive_group()
    basic_opt.add_argument('--detect', dest='bcast_addr',
                           default='192.168.10.255',
                           help='broadcast domain where UmTRX should be ' +
                                'discovered (default: 192.168.10.255)')
    basic_opt.add_argument('--umtrx-addr', dest='umtrx', const='192.168.10.2',
                           nargs='?',
                           help='UmTRX address (default: 192.168.10.2)')
    adv_opt = parser.add_mutually_exclusive_group()
    adv_opt.add_argument('--data',
                         type=lambda s: int(s, 16),
                         choices=range(0, 0x100),
                         metavar='0..0xFF',
                         help='data to be written into LMS register, hex')
    adv_opt.add_argument('--dump', action='store_true', help='dump registers')
    adv_opt.add_argument('--lms-init', action='store_true',
                         help='run init sequence for LMS')
    adv_opt.add_argument('--lms-tx-enable', type=int, choices=range(0, 2),
                         help='1 to enable TX chain, 0 to disable TX chain')
    adv_opt.add_argument('--lms-rx-enable', type=int, choices=range(0, 2),
                         help='1 to enable RX chain, 0 to disable RX chain')
    adv_opt.add_argument('--lms-auto-calibration', action='store_true',
                         help='A shorthand for ' +
                              '--lms-lpf-tuning-dc-calibration, ' +
                              '--lms-tx-lpf-dc-calibration, ' +
                              '--lms-rx-lpf-dc-calibration, ' +
                              '--lms-rxvga2-dc-calibration and ' +
                              '--lms-lpf-bandwidth-tuning.')
    adv_opt.add_argument('--lms-lpf-tuning-dc-calibration',
                         action='store_true',
                         help='DC Offset Calibration of LPF Tuning Module')
    adv_opt.add_argument('--lms-tx-lpf-dc-calibration',
                         action='store_true',
                         help='TX LPF DC Offset Calibration')
    adv_opt.add_argument('--lms-rx-lpf-dc-calibration',
                         action='store_true',
                         help='RX LPF DC Offset Calibration')
    adv_opt.add_argument('--lms-rxvga2-dc-calibration',
                         action='store_true',
                         help='RXVGA2 DC Offset Calibration')
    adv_opt.add_argument('--lms-lpf-bandwidth-tuning', action='store_true',
                         help='LPF bandwidth tuning. Specify ' +
                              '--lpf-bandwidth-code to select specific ' +
                              'LPF to tune. WARNING: Tx PLL is tuned to ' +
                              '320MHz during this procedure. Don\'t forget ' +
                              'to re-tune it back if needed.')
    adv_opt.add_argument('--lms-set-tx-pa', type=int, choices=range(0, 3),
                         help='Activate selected Tx PA, i.e. select LMS ' +
                              'output. 0 to turn off all PAs')
    adv_opt.add_argument('--lms-get-tx-pa', action='store_true',
                         help='Get active PA, i.e. active LMS output. 0 if ' +
                              'all outputs are disabled')
    adv_opt.add_argument('--lms-set-rx-lna', type=int, choices=range(0, 4),
                         help='Activate selected Rx LNA, i.e. select LMS ' +
                              'input. 0 to turn off all LNAs')
    adv_opt.add_argument('--lms-get-rx-lna', action='store_true',
                         help='Get active LNA, i.e. active LMS input. 0 if ' +
                              'all inputs are disabled')
    adv_opt.add_argument('--lms-tx-pll-tune', type=float,
                         metavar='232.5e6..3720e6',
                         help='Tune Tx PLL to the given frequency')
    adv_opt.add_argument('--lms-rx-pll-tune', type=float,
                         metavar='232.5e6..3720e6',
                         help='Tune Rx PLL to the given frequency')
    adv_opt.add_argument('--lms-set-tx-vga1-gain',
                         type=int, choices=range(-35, -3),
                         metavar='[-35..-4]', help='Set Tx VGA1 gain, in dB')
    adv_opt.add_argument('--lms-get-tx-vga1-gain',
                         action='store_true', help='Get Tx VGA1 gain, in dB')
    adv_opt.add_argument('--lms-set-tx-vga2-gain',
                         type=int,
                         choices=range(0, 26),
                         metavar='[0..25]',
                         help='Set Tx VGA2 gain, in dB')
    adv_opt.add_argument('--lms-get-tx-vga2-gain',
                         action='store_true', help='Get Tx VGA2 gain, in dB')
    adv_opt.add_argument('--lms-set-rx-vga1-gain-int',
                         type=int, choices=range(0, 121),
                         metavar='[0..120]',
                         help='Set Rx VGA1 gain raw value. 120=30dB, ' +
                              '102=19dB, 2=5dB')
    adv_opt.add_argument('--lms-get-rx-vga1-gain-int',
                         action='store_true',
                         help='Get Rx VGA1 gain raw value')
    adv_opt.add_argument('--lms-set-rx-vga1-gain', type=float,
                         metavar='5.0..30.17', help='Set Rx VGA1 gain in dB')
    adv_opt.add_argument('--lms-get-rx-vga1-gain',
                         action='store_true', help='Get Rx VGA1 gain in dB')
    adv_opt.add_argument('--lms-set-rx-vga2-gain',
                         type=int, choices=range(0, 61),
                         metavar='[0..60]',
                         help='Set Rx VGA2 gain, in dB with 3dB accuracy.')
    adv_opt.add_argument('--lms-get-rx-vga2-gain',
                         action='store_true', help='Get Rx VGA2 gain, in dB')
    adv_opt.add_argument('--lms-set-vga1-dc-i',
                         type=int, choices=range(0, 255),
                         metavar='[0..255]',
                         help='Set TxVGA1 DC shift, I channel.')
    adv_opt.add_argument('--lms-get-vga1-dc-i', action='store_true',
                         help='Get TxVGA1 DC shift, I channel')
    adv_opt.add_argument('--lms-set-vga1-dc-q',
                         type=int, choices=range(0, 255),
                         metavar='[0..255]',
                         help='Set TxVGA1 DC shift, q channel.')
    adv_opt.add_argument('--lms-get-vga1-dc-q', action='store_true',
                         help='Get TxVGA1 DC shift, q channel')
    adv_opt.add_argument('--lms-tune-vga1-dc-i', action='store_true',
                         help='Interactive tuning of TxVGA1 DC shift, ' +
                              'I channel')
    adv_opt.add_argument('--lms-tune-vga1-dc-q', action='store_true',
                         help='Interactive tuning of TxVGA1 DC shift, ' +
                              'Q channel')
    adv_opt.add_argument('--enable-loopback',
                         action='store_true', help='enable loopback')
    args = parser.parse_args()
    if args.lms is None:  # argparse do not have dependency concept for options
        if args.reg is not None or args.data is not None \
           or args.lms_tx_pll_tune is not None \
           or args.lms_rx_pll_tune is not None or args.lms_init \
           or args.lms_set_tx_pa is not None \
           or args.lms_set_rx_lna is not None \
           or args.lms_get_tx_pa or args.lms_get_rx_lna \
           or args.lms_lpf_tuning_dc_calibration \
           or args.lms_tx_lpf_dc_calibration \
           or args.lms_rx_lpf_dc_calibration \
           or args.lms_rxvga2_dc_calibration \
           ...
           or args.lms_tx_enable is not None or args.lms_tx_enable is not None:
            # gengetopt is so much better
            exit('--lms parameter is required for given options.')
    ...
    umtrx = umtrx_ctrl.detect(
        sock, args.umtrx if args.umtrx is not None else args.bcast_addr)
    if umtrx is not None:  # UmTRX address established
        if umtrx_ctrl.ping(sock, umtrx):  # UmTRX probed
            umtrx_lms_dev = umtrx_ctrl.umtrx_lms_device(
                sock, umtrx, args.lms if args.lms else 1)
            if args.lms_init:
                ...
                # 0x0f - 0.75MHz
                lpf_bw_code = args.lpf_bandwidth_code if \
                    args.lpf_bandwidth_code is not None else 0x0f
                lms_auto_calibration(umtrx_lms_dev, int(
                    args.pll_ref_clock), int(lpf_bw_code))
                ...
                lms_tx_pll_tune(umtrx_lms_dev, int(
                    args.pll_ref_clock), int(args.lms_tx_pll_tune))
            elif args.lms_rx_pll_tune is not None:
                lms_rx_pll_tune(umtrx_lms_dev, int(
                    args.pll_ref_clock), int(args.lms_rx_pll_tune))
            elif args.lms_lpf_bandwidth_tuning:
                # 0x0f - 0.75MHz
                lpf_bw_code = args.lpf_bandwidth_code if \
                    args.lpf_bandwidth_code is not None else 0x0f
                lms_lpf_bandwidth_tuning(umtrx_lms_dev, int(
                    args.pll_ref_clock), int(lpf_bw_code))
            elif args.lms_set_tx_vga1_gain is not None:
                lms_set_tx_vga1gain(umtrx_lms_dev, int(
                    args.lms_set_tx_vga1_gain))
            elif args.lms_get_tx_vga1_gain:
                gain = lms_get_tx_vga1gain(umtrx_lms_dev)
                print(gain)
            elif args.lms_set_tx_vga2_gain is not None:
                lms_set_tx_vga2gain(umtrx_lms_dev, int(
                    args.lms_set_tx_vga2_gain))
            elif args.lms_get_tx_vga2_gain:
                gain = lms_get_tx_vga2gain(umtrx_lms_dev)
                print(gain)
            elif args.lms_set_rx_vga1_gain_int is not None:
                lms_set_rx_vga1gain_int(umtrx_lms_dev, int(
                    args.lms_set_rx_vga1_gain_int))
                ...
                if args.verify:
                    ...
                    print('written 0x%02X to REG 0x%02X - %s' %
                          (vrfy, args.reg, 'OK' if vrfy == args.data
                              else 'FAIL'))
            elif args.reg is not None:
                print('read 0x%02X from REG 0x%02X' %
                      (umtrx_lms_dev.reg_read(args.reg), args.reg))
                ...
                diff = list(
                    map(lambda l1, l2: 'OK\n' if l1[1] == l2[1] else 'DIFF\n',
                        lms1, lms2))
                print(''.join(map(
                    lambda l1, l2, d: '# 0x%02X: LMS1=0x%02X \tLMS2=0x%02X' +
                                      '\t%s' % (l1[0], l1[1], l2[1], d),
                                      lms1, lms2, diff)))
            else:
