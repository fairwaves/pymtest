...
def bts_init(bts_ip, username='fairwaves'):
  ...
  # CMD57 has sloppy time synchronization, so burst timing can drift by a few symbols
...
def bts_set_slotmask(ssh, ts0, ts1, ts2, ts3, ts4, ts5, ts6, ts7):
  ...
  stdin, stdout, stderr = ssh.exec_command('./osmobts-set-slotmask.py %d %d %d %d %d %d %d %d' % (ts0, ts1, ts2, ts3, ts4, ts5, ts6, ts7))
...
def cmd57_configure(arfcn):
  ''' Configure the CMD57 '''
  dev.configure_man(ccch_arfcn=arfcn, tch_arfcn=arfcn, tch_ts=2, tsc=7, expected_power=37, tch_tx_power=-50, tch_mode='PR16', tch_timing=0)
...
def measure_tch_basic(dev):
  ...
  #       CMD57 will complain about failed mask, but we can check it ourselves
...
parser.add_argument("-p", "--cmd57-port", dest='cmd57_port', type=str, default='/dev/ttyUSB0', help="Serial port name for the CMD57 control (default: /dev/ttyUSB0)")
parser.add_argument("-a", "--arfcn", type=int, default=100, help="ARFCN to test")
